...
for i in 1:80, j in 1:80
    ...
    trans_only_tform = zeros(3,4)
    trans_only_tform[1:2, 1:2] = I(2)
    
    push!(trans_only_tforms, trans_only_tform)
    push!(src_hyb, i)
    push!(dst_hyb, j)
    push!(src_pc, i % 20)
    push!(dst_pc, j % 20)
...
@testset "Test DotAdjacencyGraph" for (i, cb) in enumerate(cbs)
    ...
    @test test_dag(300, cb, 0.05, 0.15, 0.1, 0, tforms)
    @test test_dag(300, cb, 0.05, 0.15, 0.1, 1, tforms)
    #@test test_dag_edges(300, cb)
    @test test_get_cw_pos_bnds(300, cb, 0.1, 0.3,1,ndrops=0, tforms=tforms)
    @test test_get_cw_pos_bnds(300, cb, 0.1, 0.3,1,ndrops=1, tforms=tforms)
end
...
@testset "test compute syndrome" begin
    for (i, cb) in enumerate(cbs)
        ...
        set_H(pc_matrices[i], params, cb)
        ntargets = 50000
        @test test_get_cw_pos_bnds(ntargets, cb, 0.0, 0.0, 0.0, ndrops=0, tforms=tforms)

        test_compute_syndromes(ntargets, cb, 0, tforms)
...
@testset "full decode perfect" begin
    for (i, cb) in enumerate(cbs), ntargets in [1, 10, 100]
        ...
        pnts, g = construct_test_dag(ntargets, 0, 0, 0, cb, ndrops, tforms=tforms)
...
@testset "full decode drops" begin
    for (i, cb) in enumerate(cbs), ntargets in [1, 2, 10, 100]
        ...
        decode_syndromes!(encoded, cb, H, params, tforms=tforms)
